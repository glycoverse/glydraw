...
* `draw_cartoon()` now draws normalized floating glycan parts from glyrepr and glyparse, using one SNFG bracket for either explicit or implicit attachment candidates. Repeated identical floating cartoons are merged with a count such as `3x`.
