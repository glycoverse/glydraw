...
options in `style_glydraw()` and pass the result through `style`.
...
standard reducing-end line. Use `"~"` for a wavy reducing end, pass any other
string to draw that string at the reducing end, or wrap one amino-acid
glycosite in `<site>` tags to bold and anchor it to the glycan.
...
`glydraw_colors()`. Modify entries in that palette to customize the
corresponding residue colors.
