...
when needed. Set `show_linkage` and `orient` directly; collect all visual
options in `glydraw_style()` and pass the result through `style`.
...
draw_cartoon(fucosylated, style = glydraw_style(fuc_orient = "flex"))
draw_cartoon(fucosylated, style = glydraw_style(fuc_orient = "up"))
```
...
draw_cartoon(n_core, style = glydraw_style(red_end = "~"))
draw_cartoon(n_core, style = glydraw_style(red_end = "R"))
```
...
draw_cartoon(n_core, style = glydraw_style(node_size = 1.2))
draw_cartoon(n_core, style = glydraw_style(node_size = 1.6))
```
...
```{r save-cartoon}
cartoon <- draw_cartoon(n_core, style = glydraw_style(red_end = "~"))
...
suppressMessages(
  cartoons <- export_cartoons(
    ...
    file_ext = "png",
    scale = 1.5,
    style = glydraw_style(red_end = "~", node_size = 1.1)
  )
)
